...
Usage:
  fullscope_ont_pipeline.sh \
    ...
    [--run-bambu | --run-isoquant] [--merge-annot]
...
Optional:
  ...
  --annotation-only     Reuse --fqalign and --cidmap; skip segmentation, CID,
                        and alignment steps.
  ...
  --skip-bambu-filter   Pass the complete alignment directly to Bambu.
                        This also applies to IsoQuant (legacy option name).
  --run-isoquant        Annotate reference isoforms with IsoQuant instead of Bambu.
  --isoquant-bin PATH   IsoQuant executable (default: isoquant from PATH).
  --isoquant-output-dir PATH
                        IsoQuant output root (default: OUTDIR/IsoQuant).
  --isoquant-prefix NAME
                        IsoQuant result prefix (default: sample name).
  --isoquant-genedb-output PATH
                        Local directory for IsoQuant's converted annotation DB.
  --isoquant-assignments PATH
                        Override the expected read_assignments.tsv.gz path.
  --isoquant-max-coverage-small-chr N
                        Small-chromosome coverage downsampling threshold;
                        default -1 disables downsampling to preserve all reads.
...
isoquant_max_coverage_small_chr="${ISOQUANT_MAX_COVERAGE_SMALL_CHR:--1}"
...
isoquant_r="${ISOQUANT_R:-${toolkit_root}/scripts/isoquant_read_info_to_qs.R}"
isoquant_bin="${ISOQUANT_BIN:-$(command -v isoquant || command -v isoquant.py || true)}"
...
isoquant_output_dir=""
isoquant_prefix=""
isoquant_genedb_output=""
isoquant_assignments=""
transdf_qs=""
merged_all_qs=""
merged_unique_qs=""
fragment_out=""
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        --annotation-only) annotation_only=1; shift ;;
        ...
        --run-isoquant) run_isoquant=1; shift ;;
        ...
        --isoquant-bin) isoquant_bin="$2"; shift 2 ;;
        --isoquant-r) isoquant_r="$2"; shift 2 ;;
        --isoquant-output-dir) isoquant_output_dir="$2"; shift 2 ;;
        --isoquant-prefix) isoquant_prefix="$2"; shift 2 ;;
        --isoquant-genedb-output) isoquant_genedb_output="$2"; shift 2 ;;
        --isoquant-assignments|--isoquant-read-info) isoquant_assignments="$2"; shift 2 ;;
        --isoquant-max-coverage-small-chr) isoquant_max_coverage_small_chr="$2"; shift 2 ;;
...
if [[ "$run_bambu" -eq 1 && "$run_isoquant" -eq 1 ]]; then
    echo "ERROR: --run-bambu and --run-isoquant are mutually exclusive." >&2
    exit 1
fi
if [[ "$segment_only" -eq 1 && "$annotation_only" -eq 1 ]]; then
    echo "ERROR: --segment-only and --annotation-only are mutually exclusive." >&2
    exit 1
fi
if [[ "$annotation_only" -eq 0 && "$segment_only" -eq 0 && -z "$stereoindex" ]]; then
    echo "ERROR: --sample, --outdir, and --stereoindex are required." >&2
    exit 1
fi
if [[ "$annotation_only" -eq 0 && -z "$input_bam" && -z "$raw_fq" ]]; then
...
if [[ ! "$isoquant_max_coverage_small_chr" =~ ^-?[0-9]+$ ||
      "$isoquant_max_coverage_small_chr" -eq 0 ||
      "$isoquant_max_coverage_small_chr" -lt -1 ]]; then
    echo "ERROR: --isoquant-max-coverage-small-chr must be -1 or a positive integer." >&2
    exit 1
fi

out_prefix="${out_prefix:-$sample}"
bambu_sample="${bambu_sample:-$sample}"
isoquant_prefix="${isoquant_prefix:-$sample}"

mkdir -p "${outdir}/Index" "${outdir}/CIDmap" "${outdir}/CIDextract" "${outdir}/Alignment" "${outdir}/Fqsegment" "${outdir}/raw_fastq" "${outdir}/Bambu" "${outdir}/IsoQuant"
...
cidindex_txt="${cidindex_txt:-${outdir}/Index/$(basename "${stereoindex:-stereoindex}").txt}"
stereo_base="$(basename "${stereoindex:-stereoindex}")"
...
isoquant_output_dir="${isoquant_output_dir:-${outdir}/IsoQuant}"
isoquant_genedb_output="${isoquant_genedb_output:-${TMPDIR:-/tmp}/fullscope_isoquant_genedb_${sample}}"
isoquant_assignments="${isoquant_assignments:-${isoquant_output_dir}/${isoquant_prefix}/${isoquant_prefix}.read_assignments.tsv.gz}"
if [[ "$run_isoquant" -eq 1 ]]; then
    transdf_qs="${transdf_qs:-${isoquant_output_dir}/${isoquant_prefix}_trans_total_anno.qs}"
    merged_all_qs="${merged_all_qs:-${outdir}/${sample}_isoquant_fsraw_merged_data.qs}"
    merged_unique_qs="${merged_unique_qs:-${outdir}/${sample}_isoquant_fsraw_merged_data_uniquereads.qs}"
else
...
ulimit -n 65535 || true
if [[ "$annotation_only" -eq 0 && "$raw_fq_from_bam" -eq 1 ]]; then
    need_cmd bedtools
fi
if [[ "$annotation_only" -eq 0 && "$segment_only" -eq 0 ]]; then
    need_cmd minimap2
    need_cmd samtools
...
if [[ "$run_bambu" -eq 1 || "$run_isoquant" -eq 1 ]]; then
    [[ -n "$genome" ]] || { echo "ERROR: --genome or GENOME_FA is required for isoform annotation." >&2; exit 1; }
    [[ -n "$gtf" ]] || { echo "ERROR: --gtf or GENES_GTF is required for isoform annotation." >&2; exit 1; }
    need_file "$genome"
    need_file "$gtf"
fi
if [[ "$annotation_only" -eq 0 ]]; then
...
if [[ "$run_isoquant" -eq 1 ]]; then
    [[ -n "$isoquant_bin" ]] || { echo "ERROR: IsoQuant is required for --run-isoquant; use --isoquant-bin." >&2; exit 1; }
    need_file "$isoquant_bin"
    need_file "$isoquant_r"
    [[ -n "$rscript_bin" ]] || { echo "ERROR: Rscript is required for IsoQuant assignment conversion." >&2; exit 1; }
    need_file "$rscript_bin"
fi
...
if [[ "$annotation_only" -eq 0 && "$skip_fastq" -eq 0 ]]; then
    if [[ ! -s "$raw_fq" ]]; then
        need_file "$input_bam"
...
samtools index "$fqalign"
    ...
    need_file "$cidmap"
    need_cmd samtools
    samtools quickcheck "$fqalign"
    log "Annotation-only mode: reusing alignment $fqalign and CID map $cidmap"
fi

if [[ "$run_bambu" -eq 1 || "$run_isoquant" -eq 1 ]]; then
    if [[ "$skip_bambu_filter" -eq 1 ]]; then
        bambu_input_bam="$fqalign"
        log "Skipping the annotation alignment filter by request"
    else
        bambu_input_bam="$bambu_bam"
        if [[ -s "$bambu_input_bam" && -s "${bambu_input_bam}.bai" ]] &&
           samtools quickcheck "$bambu_input_bam"; then
            log "Reusing annotation-filtered alignment: $bambu_input_bam"
        else
            ...
            log "Filtering alignment for isoform annotation: primary mapped reads, query length ${bambu_min_read_length}-${bambu_max_read_length}"
            ...
            log "Annotation alignment filter counts: $(tr -d '\n' < "$filter_counts")"
        fi
    fi
    if [[ "$run_bambu" -eq 1 ]]; then
        log "Running Bambu"
        ...
        mkdir -p "$isoquant_output_dir" "$isoquant_genedb_output"
        log "Running IsoQuant reference-annotation quantification"
        PATH="$(dirname "$isoquant_bin"):${PATH}" "$isoquant_bin" \
            --reference "$genome" \
            --genedb "$gtf" \
            --bam "$bambu_input_bam" \
            --data_type nanopore \
            ...
            --max_coverage_small_chr "$isoquant_max_coverage_small_chr" \
            --threads "$threads" \
            --prefix "$isoquant_prefix" \
            --output "$isoquant_output_dir" \
            --genedb_output "$isoquant_genedb_output"
        need_file "$isoquant_assignments"
        log "Converting IsoQuant read assignments to Fullscope QS"
        "$rscript_bin" "$isoquant_r" "$isoquant_assignments" "$transdf_qs" "$isoquant_prefix"
    fi
fi
